...
!      write(*,*) 'iSubbasin, ihru = ', i,ihru
      iPAR = 15  !!spcon
      CALL par_bsn_new(spcon,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))

      iPAR = 16  !!spexp
      CALL par_bsn_new(spexp,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 17 !!prf_bsn
      CALL par_bsn_new(prf_bsn,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 18  !!adj_pkr
      CALL par_bsn_new(adj_pkr,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
      
      iPAR = 23  !!rsdco
      CALL par_bsn_new(rsdco,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 24  !!nperco
      CALL par_bsn_new(nperco,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 25  !!n_updis
      CALL par_bsn_new(n_updis,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 31  !!pperco
      CALL par_bsn_new(pperco,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 32  !!phoskd
      CALL par_bsn_new(phoskd,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
     
      iPAR = 33  !!psp
      CALL par_bsn_new(psp,sGLB%xua(iPAR),
     &                 sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR))
...
!!    mike van liew additions for basins.bsn
	...
	!  stop
      ...
      call caps(petfile)
      call caps(wwqfile)
      open (101,file=trim(sGLB%dir_swatio)//trim(wwqfile))
...
!!    at some future time)
      ...
      !!=====================
      if (cswat == 2) then
      open (98,file=trim(sGLB%dir_swatio)//
     & "cswat_profile.txt",recl=356)
     ...
     &'sol_solp' 
      ...
      open (100,file=trim(sGLB%dir_swatio)//
     & "cswat_daily.txt",recl=786)
