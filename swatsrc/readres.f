...
!      iPAR = 15
!!      write(*,*)sGLB%xua(iPAR)
!      CALL par_res_new(evrsv(i),i,sGLB%xua(iPAR),
!     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
!     &             sGLB%icode_res)
!!      write(*,*)"ires=", i,"EVRSV=",evrsv(i)
...
!        call caps(resdayo)
!!SCE_BEGIN
        open(350+i,file=trim(sGLB%dir_obs)//trim(resdayo))  !!open (350+i,file=resdayo)
        read (350+i,1000) titldum
        
        sYear = trim(titldum)
        sYear = sYear(1:4)
        read(sYear,*)jYear  !!the beginning year of the observations
        do j=jYear,(sGLB%iyear0 - 1)  !!skip the data before sGLB%iyear0
...
!! read in monthly release data
      if (iresco(i) == 1) then
        call caps(resmono)
        open(101,file='userdoc/aOBS/'//trim(resmono)) !!open (101,file=resmono)
