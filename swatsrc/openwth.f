...
!! open precip files and read elevation
     ...
     !!     open (100+j,file=rfile(j),recl=800)
          open (100+j,file=
     &     trim(sGLB%dir_swatio)//trim(rfile(j)),recl=1850)
  ...
  !!        open (118+j,file=tfile(j),recl=800)
          open (118+j,file=trim(sGLB%dir_swatio)//trim(tfile(j)),
     &     recl=20000)
   ...
   !!       open (137,file=slrfile,recl=800)
          open (137,file=trim(sGLB%dir_swatio)//trim(slrfile),
     &     recl=15000)
  ...
  !!        open (138,file=rhfile,recl=800)
          open (138,file=trim(sGLB%dir_swatio)//trim(rhfile),
     &     recl=15000)
   ...
   !!       open (139,file=wndfile,recl=800)
          open (139,file=trim(sGLB%dir_swatio)//trim(wndfile),
     &     recl=15000)
          read (139,5000) titldum
        end if

        if (petfile /= '             ') then
          open (140,file=trim(sGLB%dir_swatio)//trim(petfile))
