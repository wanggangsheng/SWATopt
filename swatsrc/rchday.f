...
!!  compute month and day given julian day
        ...
        if (ipdvar(1) > 0) then
          do kk=1,nstep
           ...
           if (iscen == 1 .and. isproj == 0) then
             write (7,5001) j,',',subgis(j),',', iida,',', kk,',',         
     &             rch_dakm(j),',', (pdvr(ii),',', ii = 1, itotr),j
	    ...
	    if (iscen == 1 .and. isproj == 0) then
           do kk=1,nstep
             write (7,5001) j,',', subgis(j),',', iida,',', kk,',',         
     &             rch_dakm(j),',', (rchhr(ii,j,kk),',', ii = 1, 7), j
	     end do
	     endif
	  endif
      else
     ...
     &           ',', rch_dakm(j),',', (pdvr(ii),',', ii = 1, itotr), j
            if(icalen == 1)write (7,5002) j,',', subgis(j),',', 
     &              i_mo, ',',icl(iida),',',iyr,',', rch_dakm(j),',', 
     &              (pdvr(ii),',', ii = 1, itotr)
!!    added for binary files 3/25/09 gsm line below and write (77777
     ...
     &                                 (pdvr(ii),',', ii = 1, itotr), j
          ...
          if(icalen == 0)write (7,6000) j,',',subgis(j),',', iida,',', 
     &               rch_dakm(j),',', (pdvr(ii),',', ii = 1, itotr),iyr 
          if (icalen == 1) write (7,6002) j,',', subgis(j),',', 
     &          i_mo,',', icl(iida),',',iyr,',', rch_dakm(j),',',
...
!!    added for binary files 3/25/09 gsm line below and write (77777
     ...
     &                                      (pdvar(ii),',', ii = 1, 45) 
          ...
          if (icalen == 0)write(7,6000) j,',', subgis(j),',', 
     &                              iida,',', rch_dakm(j),',',
...
! 6002 format ('REACH ',i4,1x,i8,1x,i2,1x,i2,1x,i4,1x,47e12.4,1x,i4)
 ...
 5001 format ('REACH,',i4,a1,1x,i8,a1,1x,i5,a1,1x,i5,a1,47(e12.4,a1),i6)
 5002 format ('REACH,',i4,a1,1x,i8,a1,2x,i2,a1,1x,i2,a1,1x,i4,a1,1x,
     &                  47(e12.4,a1))
 6000 format ('REACH,',i4,a1,1x,i8,a1,1x,i5,a1,47(e12.4,a1),1x,i4)
 6002 format ('REACH,',i4,a1,1x,i8,a1,1x,i2,a1,1x,i2,a1,1x,i4,a1,1x,
     &                  47(e12.4,a1),1x,i4)
      end