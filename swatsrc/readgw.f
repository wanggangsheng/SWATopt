...
!      write(*,*) 'iSubbasin, ihru = ', i,ihru
      iPAR = 7
      CALL par_new(alpha_bf(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)

      
      iPAR = 8
      CALL par_new(delay(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
      
      iPAR = 9
      CALL par_new(rchrg_dp(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 10
      CALL par_new(gw_revap(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
      
      iPAR = 11
      CALL par_new(gw_spyld(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 27  !!shallst_n
      CALL par_new(shallst_n(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
