...
!     septic database file
      ...
      open (23,file=trim(sGLB%dir_swatio)//"file.cio")
      ...
      read (23,6000) titldum
      read (23,6000) titldum
      read (23,*) nstep       !!IDT : Rainfall data time step
      read (23,6000) titldum
      read (23,6000) titldum
      read (23,*) nrgage
      read (23,*) nrtot
...
!! calculate number of records in plant growth database
      eof = 0
      mcrdb = 0
      open (29,file=trim(sGLB%dir_swatio)//trim(plantdb))
      do
        ic = 0
...
!! calculate number of records in urban database
      eof = 0
      mudb = 0
      open (8,file=trim(sGLB%dir_swatio)//trim(urbandb))
      do
        iunum = 0
...
!     septic database
      if (septdb /= '             ') then
         open (171,file=trim(sGLB%dir_swatio)//trim(septdb)) !! CS
         do jj = 1,4
...
!! calculate number of records in fertilizer database
      eof = 0
      mfdb = 0
      open (7,file=trim(sGLB%dir_swatio)//trim(fertdb))
      do
        ifnum = 0
...
!! calculate number of records in pesticide database
      eof = 0
      mpdb = 0
      open (31,file=trim(sGLB%dir_swatio)//trim(pestdb))
      do
        ipnum = 0
...
!! calculate number of records in tillage database
      eof = 0
      mtil = 0
      open (30,file=trim(sGLB%dir_swatio)//trim(tilldb))
      do
        itnum = 0
...
!! process .fig file
      ...
      pstflg = 0
      mhru1 = 1
      open (27,file=trim(sGLB%dir_swatio)//trim(figfile))
      do while (icd > 0)
        read (27,5002) a
        if (a /= "*") then
          backspace 27
          ...
          case (1)                      !! icd = 1  SUBBASIN command
            ...
            call caps(subfile)
            open (25,file=trim(sGLB%dir_swatio)//trim(subfile))
            do j = 1,52 
...
!! calculate max number of forecast stations
      ...
      if (fcstfile /= '             ') then
        fcsttot = 0
        open (12,file=trim(sGLB%dir_swatio)//trim(fcstfile))
