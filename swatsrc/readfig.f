...
!          write(*,*)'icodes: ', icodes(idum)  
          ...
          select case(icodes(idum))
            ...
            case (1)  !! icode = 1  SUBBASIN command
              ...
              open (101,file=trim(sGLB%dir_swatio)//trim(subfile))
              call readsub
              nhru = nhru + hrutot(i)
...
!!            assume subbasin is the same number as the reach (if zero)              
              ...
              irch = 0
              irch = inum1s(idum)
              open (103,file=trim(sGLB%dir_swatio)//trim(rtefile))
              open (104,file=trim(sGLB%dir_swatio)//trim(swqfile))
              call readrte
              call readswq
            ...
            case (3)  !! icode = 3  ROUTE RESERVOIR command
              ...
              i = 0
              i = inum1s(idum)
              open (105,file=trim(sGLB%dir_swatio)//trim(resfile))
              call readres
              if (lwqfile /= '             ') then
                open (106,file=trim(sGLB%dir_swatio)//trim(lwqfile))
                call readlwq
              end if
     ...
     &		    mo_transe(inum5s(idum)), ih_tran(inum5s(idum))
            ...
            case (6)  !! icode = 6  RECHOUR command: read in hourly values
              ...
              call caps(hour_in)
              open (200+inum1s(idum),
     &         file=trim(sGLB%dir_swatio)//trim(hour_in),recl=350)
              do ii = 1, 6
            ...
            i = 0
            i = inum1s(idum)
            open (107,file=trim(sGLB%dir_swatio)//trim(month_in),
     &       recl=350)
            call readmon
            ...
            i = 0
            i = inum1s(idum)
            open (108,file=trim(sGLB%dir_swatio)//trim(year_in),
     &       recl=350)
            call readyr
     ...
     &         file=trim(sGLB%dir_swatio)//trim(day_in),recl=350)
            ...
            i = 0
            i = inum1s(idum)
            open (109,file=trim(sGLB%dir_swatio)//trim(annual_in),
     &       recl=350)
            call readcnst
...
!! code to read from apex output file
      ...
      !      i = 0
      !      i = inum1s(idum)
            open (112+inum1s(idum),
     &       file=trim(sGLB%dir_swatio)//trim(apex_in),recl=350)
	      do ii = 1, 9 
...
!                  write(*,*)50+inum1s(idum)
     ...
     &           file=trim(sGLB%dir_swatio)//trim(day_in),recl=350)
            ...
            case (17)  !! icode = 17  ROUTING UNIT command
              ...
              open (113,file=trim(sGLB%dir_swatio)//trim(rufile))
              call readru
              close(113)
