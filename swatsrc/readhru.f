...
!!SCE_BEGIN
      if(sGLB%iModel.gt.0) then  !!SCEUA
!      par_new(par,isub_cur,x3,ifopt,ivar3,nsub3,isub3)
      
!      call SGLB_WRITE(sGLB)
!      write(*,*)iPAR,sGLB%xua(iPAR),
!     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
!     &             sGLB%nsub_opt,sGLB%isub_opt
! i: curren subbasin ID in SWAT loop
      iPAR = 2
      CALL par_new(escohru,i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)

      iPAR = 3
      CALL par_new(epcohru,i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 4
      CALL par_new(ov_n(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 28  !!erorgn
      CALL par_new(erorgn(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 37  !!erorgp
      CALL par_new(erorgp(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
