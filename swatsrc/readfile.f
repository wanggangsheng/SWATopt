...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &    pestdb, fertdb, fcstfile
      ...
      open (101,file=trim(sGLB%dir_swatio)//"file.cio")
...
!! Read general information/watershed configuration
      ...
      call caps(figfile)
      open (102,file=trim(sGLB%dir_swatio)//trim(figfile))
...
!     & Soil Layers')
      ...
      call caps(fcstfile)
      if (fcstfile /= '             ') then 
        open (109,file=trim(sGLB%dir_swatio)//trim(fcstfile))
      else
        fcstyr = 0
        fcstday = 0
      end if
...
!!Open watershed modeling option file
      ...
      call caps(bsnfile)
      open (103,file=trim(sGLB%dir_swatio)//trim(bsnfile))
...
!!Open database files
      ...
      call caps(fertdb)
      call caps(urbandb)
      open (104,file=trim(sGLB%dir_swatio)//trim(plantdb))
      open (105,file=trim(sGLB%dir_swatio)//trim(tilldb))
      open (106,file=trim(sGLB%dir_swatio)//trim(pestdb))
      open (107,file=trim(sGLB%dir_swatio)//trim(fertdb))
      open (108,file=trim(sGLB%dir_swatio)//trim(urbandb))
...
!!   isol = 1 print file
      ...
      if (isol == 1) then
         open (121,file=trim(sGLB%dir_swatio)//'output.snu')
         write (121,12222) 
...
!!    read from readlup (landuse update file)
       open (122,file=trim(sGLB%dir_swatio)//'lup.dat')
...
!!    output by elevation band to (formerly 'snowband.out')
       read (101,*,iostat=eof) isnow
	    if (isnow == 1) then
         open (115,file=trim(sGLB%dir_swatio)//'output.snw')
         write (115,1010)
       end if
...
!       itoth = mhruo
      ...
      !!Open output files
      open (24,file=trim(sGLB%dir_swatio)//"input.std")
      open (26,file=trim(sGLB%dir_swatio)//"output.std")

      open (28,file=trim(sGLB%dir_swatio)//"output.hru",recl=1500)
      if (ia_b == 1) then 
        open (33333,file=trim(sGLB%dir_swatio)//"outputb.hru",
     &   form='unformatted')
      end if
      open (30,file=trim(sGLB%dir_swatio)//"output.pst",recl=600)
      open (31,file=trim(sGLB%dir_swatio)//"output.sub",recl=600)
      if (ia_b == 1) then
        open (66666,file = trim(sGLB%dir_swatio)//"outputb.sub",
     &   form = 'unformatted')
      end if
      open (7,file=trim(sGLB%dir_swatio)//"output.rch",recl=800)
      open (8,file=trim(sGLB%dir_swatio)//"output.rsv",recl=800)
      if (ia_b == 1) then
        open (77777,file = trim(sGLB%dir_swatio)//"outputb.rch",
     &   form = 'unformatted')
      end if
      
!!    sediment routing output file
      open (84,file=trim(sGLB%dir_swatio)//"output.sed",recl=800)
...
1080  format (t8,'RCH',t17,'GIS',t23,'MON',t31,'AREAkm2',               
     ...
     &'CH_BEDtons',t232,'CH_DEPtons',t244,'FP_DEPtons',t259,'TSSmg/L')
      ...
      open (77778,file = trim(sGLB%dir_swatio)//"bmp-sedfil.out") !jaehak temp urban print out
      write(77778,'(a46)') 'Sed-Fil Basins Configuration'   
      write(77778,'(a46)') ''   ! 
       !retention-irrigation output
      open (77779,file = trim(sGLB%dir_swatio)//"bmp-ri.out") !jaehak temp urban print out
...
!! srin output file from watqual.f  
      if (ihumus ==1) then
        open (82,file=trim(sGLB%dir_swatio)//'output.wql')
        write (82,6000)
...
!! mauro/jerry whittaker hourly output file
      if (iphr > 0) then
        open (83,file=trim(sGLB%dir_swatio)//'hourq.out')
        write (83,6001) 
...
!!darrell output files added for interface plotting
      open (11,file=trim(sGLB%dir_swatio)//'rch.dat')
      open (12,file=trim(sGLB%dir_swatio)//'hru.dat')
      open (13,file=trim(sGLB%dir_swatio)//'sub.dat')
      open (14,file=trim(sGLB%dir_swatio)//'rsv.dat')
!!darrell output files added for interface plotting
      open (11123,file=trim(sGLB%dir_swatio)//'hyd.out')
      open (16,file=trim(sGLB%dir_swatio)//'chan.deg')
!!    open (17,file='wbl.out')
      open (18,file=trim(sGLB%dir_swatio)//'swat.qst')
!! output amount of water stored in the soil layer (formerly 'soilst.out')
      if (isto > 0) then
        open (129,file=trim(sGLB%dir_swatio)//'output.swr')
        write (129,5001) 
...
!! Output daily streamflow velocity for each channel (subbasin)
      if (itemp == 1) then
         open (141,file=trim(sGLB%dir_swatio)//'output.vel')
         write (141,4999)
 4999     format(t17,'CH_VEL',/,t3,'Day',t7,'Year',t18,'(m/s)')
         open (142,file=trim(sGLB%dir_swatio)//'output.dep')
          write (142,4998)
...
!  0=no print 1=print
       read (101, *,iostat=eof) imgt
	    if (imgt==1) then
         open (143, file=trim(sGLB%dir_swatio)//"output.mgt",
     &    recl=600)
         write (143,999)
...
!!0-no print, 1-print
        ...
        if (iwtr == 1) then
          open (29,file=trim(sGLB%dir_swatio)//"output.wtr",recl=800)
! write statement added for Aziz (06/25/09)
          open (125,file=trim(sGLB%dir_swatio)//'output.pot')
          write (125, 1000) 
        end if
...
!!!!! if icalen == 1 (print month/day/year) - force iprint to be daily  <--nubz asked srin 06/11/2012
      ...
      if (isproj == 1) then
        open (19,file=trim(sGLB%dir_swatio)//"output2.std")
        open (20,file=trim(sGLB%dir_swatio)//"output2.rch",recl=600)
        open (21,file=trim(sGLB%dir_swatio)//"output2.hru",recl=800)
        open (22,file=trim(sGLB%dir_swatio)//"output2.rsv",recl=800)
      end if

      if (cswat == 1) then
	  open (100,
     &     file=trim(sGLB%dir_swatio)//"cswat_profile.txt",recl=280)
	  write (100,*) 'year',';','day',';','hru',';','cmass',';',
     &'sol_rsd',';','mancmass'
      end if

!! septic result  J.Jeong Feb2009
      open (173,file=trim(sGLB%dir_swatio)//'septic.out')
