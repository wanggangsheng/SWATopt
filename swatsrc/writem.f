...
!! if last day of month or last day in last year
      ...
      if (i_mo /= mo_chk .or. (curyr == nbyr .and. i == idal)) then  !!month for next day .NE. month for current day: current day is the last day of the month; .or. last day of last year
        mo_atmo = mo_atmo + 1
...
!! WGS: correct error: if use line 219 & 224, for nonleap year, idlast = 27, should be 28
!! WGS          if (leapyr == 1 .and. mo_chk == 2) idlast = idlast - 1
...
!          write(*,*)sGLB%itype_opt
          nnsub = sGLB%nsub_opt
          jjsub = sGLB%icode_sub
          jjmon = sGLB%imon_sim
          cflux2conc = 1.0E+6/Real(24*3600*idlast)  !!~0.386, convert tons/month to mg/L 
          if(sGLB%itype_opt.eq.11) then  !!HUC8_RUNOFF; WYLD, PCP
            sGLB%data_sim(1:nnsub,jjmon) 
     &               = submono(4,sGLB%isub_opt(1:nnsub))
            sGLB%data_sim(nnsub+1:nnsub*2,jjmon) 
     &               = submono(1,sGLB%isub_opt(1:nnsub))
     
          elseif(sGLB%itype_opt.eq.10) then  !!monthly streamflow; (1)qsim,(2)q_flowin,(3)PCP	
            sGLB%data_sim(1,jjmon) = rchmono(2,jjsub)/Real(idlast) !!flow;
            sGLB%data_sim(2,jjmon) = rchmono(1,jjsub)/Real(idlast) !!flow_in;
...
!     &       rchmono(33,jjsub),rchmono(7,jjsub)  !!NH4, ORGN
            ...
            sGLB%data_sim(3,jjmon) = rchmono(9,jjsub)+rchmono(18,jjsub)  !!TP = ORGP + SOLP (kg)
            sGLB%data_sim(4,jjmon) = rchmono(16,jjsub)  
     &          + rchmono(35,jjsub)+rchmono(33,jjsub)+rchmono(7,jjsub)!!TN = NO3 + NO2+ NH4+ORGN (kg)
            sGLB%data_sim(5,jjmon) = rchmono(16,jjsub)+rchmono(35,jjsub) !!NO3 + NO2 (kg)
            sGLB%data_sim(6,jjmon) = rchmono(16,jjsub)  !!NO3 (kg)
                         
            do j = 3,6
                sGLB%data_sim(j,jjmon) = sGLB%data_sim(j,jjmon)*0.001   !!kg -> ton
            end do
            
            if(sGLB%itype_opt.ge.30.and.sGLB%itype_opt.le.39) then  !!nutrients in concentration units of [mg/L]
                do j = 2,6
                    if(sGLB%data_sim(1,jjmon).gt.1.0E-6) then
                        sGLB%data_sim(j,jjmon) = sGLB%data_sim(j,jjmon)
     &                              / sGLB%data_sim(1,jjmon)*cflux2conc   !!ton/month->mg/L
                    else
                        sGLB%data_sim(j,jjmon) = 0.0
                    end if
                end do
            end if
!            write(*,*)sGLB%imon_sim,sGLB%icode_sub,
!     &       rchmono(16,sGLB%icode_sub),rchmono(35,sGLB%icode_sub),
!     &       rchmono(33,sGLB%icode_sub),rchmono(7,sGLB%icode_sub)
!           write(*,*)sGLB%imon_sim,idlast,sGLB%data_sim(:,sGLB%imon_sim)
          end if
      end if
!!SCE_END  
!!---------------------------------------------------------------------        
...
!!SCE_BEGIN
!          if(curyr.gt.sGLB%nyear_warmup) then
!            write(*,*)'writem.f: ',mdays,sGLB%imon_sim
!          end if
!!SCE_END
...
! 6800 format ('RES   ',i8,1x,i4,41e12.4,1x,i4)
 5800 format ('RES,  ',i8,a1,1x,i4,a1,41(e12.4,a1))
 6800 format ('RES,  ',i8,a1,1x,i4,a1,41(e12.4,a1),1x,i4)
