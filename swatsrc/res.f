...
!!---------------------------------------------------------------------
!!SCE_BEGIN
!      write(*,*)'iDaySim = ', idaysim
      if(jres.eq.sGLB%icode_res.and.curyr.gt.sGLB%nyear_warmup) then 
            sGLB%data_obs(2,sGLB%iday_sim) = flw !!sGLB%resq_obs(sGLB%iday_sim) ;wgs,origin: resflwo/86400.0  now: flwin
            sGLB%data_sim(2,sGLB%iday_sim) = resflwo/86400.0 !!sGLB%resq_sim(sGLB%iday_sim); wgs,origin: resflwo/86400.0  now: flwin
            sGLB%data_sim(1,sGLB%iday_sim) = res_vol(jres)/1.0e4        !!sGLB%resv_sim(sGLB%iday_sim);wgs2006-08-04
            
            sGLB%data_sim(3,sGLB%iday_sim) = resflwi/1.0e4 !!resqin; inflow (10^4 m3)
            sGLB%data_sim(4,sGLB%iday_sim) = respcp/1.0e4 !!respcp,(10^4 m3); sub_subp(res_sub(jres)) !!(mm)
            sGLB%data_sim(5,sGLB%iday_sim) = resev/1.0e4  !!reseva,(10^4 m3); evrsv(jres) * pet_day   !!(mm)
            sGLB%data_sim(6,sGLB%iday_sim) = ressa                    !!ressa, (ha)
!            write(*,*)sGLB%data_sim(:,sGLB%iday_sim)
!            flgwus(idaysim) = xx/10000.
      end if !!jres.eq.icoderes      
!!SCE_END   
!!---------------------------------------------------------------------      
      return
 5000 format (f8.2)
