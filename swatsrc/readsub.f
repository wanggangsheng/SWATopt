...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &  sub_smfmn(1,i), sub_sftmp(1,i), sub_smtmp(1,i), sub_timp(1,i)
      ...
      read (101,5300) wgnfile
        call caps(wgnfile)
        open (114,file=trim(sGLB%dir_swatio)//trim(wgnfile))
      read (101,*) fcst_reg(i)
      read (101,5100) titldum
      read (101,5100) titldum
      ...
      read (101,5300) pndfile
        call caps(pndfile)
        open (104,file=trim(sGLB%dir_swatio)//trim(pndfile))
      read (101,5100) titldum
      read (101,5300) wusfile
        call caps(wusfile)
        open (105,file=trim(sGLB%dir_swatio)//trim(wusfile))
      read (101,5100) snofile
      if(snofile /='             ' .or. snofile /= 'Climate Change')then
        if (snofile /='             ') then
          if (snofile /= 'Climate Change') then
            open (113,file=trim(sGLB%dir_swatio)//trim(snofile))
            call caps (snofile)
            call readsno
          endif
        endif
...
!         irip(ihru) = ir
     ...
     & opsfile, septfile, sdrfile, ils2(ihru)
          ...
          if (septfile /='             ') then
            call caps (septfile)
            open (172,file=trim(sGLB%dir_swatio)//trim(septfile),
     &       status='old')
            isep_hru(ihru) = 1
            call readsepticbz
          end if
          if (sdrfile /= '             ') then
	      call caps(sdrfile)
	      open (112,file=trim(sGLB%dir_swatio)//trim(sdrfile))
	      call readsdr
	    end if
          
          open (106,file=trim(sGLB%dir_swatio)//trim(chmfile))
          open (107,file=trim(sGLB%dir_swatio)//trim(solfile))
          open (108,file=trim(sGLB%dir_swatio)//trim(hrufile))
          open (109,file=trim(sGLB%dir_swatio)//trim(mgtfile))
          open (110,file=trim(sGLB%dir_swatio)//trim(gwfile))
          call readhru
          call readchm
          call readmgt
          call readsol
          ...
          if (opsfile /= '             ') then
            call caps(opsfile)
            open (111,file=trim(sGLB%dir_swatio)//trim(opsfile))
            call readops
          end if
