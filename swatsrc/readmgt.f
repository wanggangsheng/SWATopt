...
!      write(*,*)"iSub,ihru,OLD_par = ",i,ihru, cn2(ihru)
!      write(*,*)cn2(ihru),i,sGLB%xua(iPAR),
!     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
!     &             sGLB%nsub_opt,sGLB%isub_opt
      iPAR = 1
      CALL par_new(cn2(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
!      write(*,*)"iSub,ihru,new_par = ",i,ihru, cn2(ihru)
     
      iPAR = 22
      CALL par_new(biomix(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
...
!              if(i.eq.sGLB%isub_opt(jua)) then    !!"i" is a global variable for subbasin ID in SWAT
!                  if(sGLB%ivar_par(iPAR).eq.2) then  !relative value
!                    cn2(ihru) = cn2(ihru)*sGLB%xua(iPAR)  !relative value, multiplied by a factor
!                  else
!                    cn2(ihru) = sGLB%xua(iPAR) !absolute value
!                  end if
!              end if
!          end do
!      end if !!if(sGLB%iopt_par(1).gt.0)