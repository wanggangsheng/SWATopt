...
!      write(*,*) 'iSubbasin, ihru = ', i,ihru
      iPAR = 5
      CALL par_new(ch_n(2,irch),irch,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)

      iPAR = 6
      CALL par_new(ch_k(2,irch),irch,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 19 !!CH_COV
      CALL par_new(ch_cov2(irch),irch,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)  
     
      iPAR = 20 !!CH_EROD
      CALL par_new(ch_cov1(irch),irch,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
