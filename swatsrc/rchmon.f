...
!!SCE_BEGIN; ATTENTION: MOVED TO "writem.f"
!      if(curyr.gt.sGLB%nyear_warmup) then
!!          write(*,*)sGLB%itype_opt
!          if(sGLB%itype_opt.eq.11) then  !!WYLD, PCP
!            sGLB%data_sim(1:sGLB%nsub_opt,sGLB%imon_sim) 
!     &               = submono(4,sGLB%isub_opt(1:sGLB%nsub_opt))
!            sGLB%data_sim(sGLB%nsub_opt+1:sGLB%nsub_opt*2,sGLB%imon_sim) 
!     &               = submono(1,sGLB%isub_opt(1:sGLB%nsub_opt))
!          elseif(sGLB%itype_opt.ge.20.and.sGLB%itype_opt.le.39) then  !!(1)Flow_cms, (2)Sed_ton, (3)TP_ton, (4)TN_ton, (5)NO2+3_ton, (6)NO3_ton
!            sGLB%data_sim(1,sGLB%imon_sim) = 
!     &          rchmono(2,sGLB%icode_sub)/Real(mdays) !!flow; idlast = # of days in current month
!            sGLB%data_sim(4,sGLB%imon_sim) = rchmono(16,sGLB%icode_sub)  
!     &         + rchmono(35,sGLB%icode_sub)+ rchmono(33,sGLB%icode_sub)
!     &         + rchmono(7,sGLB%icode_sub)!!TN = NO3 + NO2+ NH4+ORGN
!            sGLB%data_sim(3,sGLB%imon_sim) = rchmono(9,sGLB%icode_sub)  
!     &         + rchmono(18,sGLB%icode_sub)  !!TP = ORGP + SOLP
!            sGLB%data_sim(2,sGLB%imon_sim) = rchmono(4,sGLB%icode_sub)  !!Sediment
!            sGLB%data_sim(5,sGLB%imon_sim) = rchmono(16,sGLB%icode_sub)  
!     &           + rchmono(35,sGLB%icode_sub)                            !!NO3 + NO2
!            sGLB%data_sim(6,sGLB%imon_sim) = rchmono(16,sGLB%icode_sub)  !!NO3
!            do j = 3,6
!            sGLB%data_sim(j,sGLB%imon_sim) = 
!     &           sGLB%data_sim(j,sGLB%imon_sim)*0.001   !!kg -> ton
!            end do
!           write(*,*)sGLB%imon_sim,mdays,sGLB%data_sim(:,sGLB%imon_sim)  
!          end if
!      end if
!!SCE_END
