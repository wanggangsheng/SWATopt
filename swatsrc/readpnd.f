...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      open (104,file=trim(sGLB%dir_swatio)//trim(dpd_file))
      read (104,5100,iostat=eof) titldum
      if (eof < 0) exit
...
!!  END DETENTION POND FILE
      ...
      open (104,file=trim(sGLB%dir_swatio)//trim(wpd_file))
      read (104,5100,iostat=eof) titldum
      if (eof < 0) exit
...
!! end wet pond (.wpd file)
      ...
      open (104,file=trim(sGLB%dir_swatio)//trim(rib_file))
      read (104,5100,iostat=eof) titldum
      if (eof < 0) exit
...
!! end .rib file
      ...
      open (104,file=trim(sGLB%dir_swatio)//trim(sfb_file))
