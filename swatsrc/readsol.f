...
!!      write(*,*)'usle_k = ',i,ihru,usle_k(ihru)
...
!!SCE_BEGIN
      if(sGLB%iModel.gt.0) then
      !2 parameters, multiple layers
!      write(*,*) 'iSubbasin, ihru = ', i,ihru
      
!      write(*,*)sol_awc(1:nly,ihru),nly,i,sGLB%xua(iPAR),
!     &              sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
!     &              sGLB%nsub_opt,sGLB%isub_opt
      iPAR = 12
      CALL pars_new(sol_awc(1:nly,ihru),nly,i,sGLB%xua(iPAR),
     &              sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &              sGLB%nsub_opt,sGLB%isub_opt)

      iPAR = 13
      CALL pars_new(sol_k(1:nly,ihru),nly,i,sGLB%xua(iPAR),
     &              sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &              sGLB%nsub_opt,sGLB%isub_opt)
     
      iPAR = 21 !!usle_k
      CALL par_new(usle_k(ihru),i,sGLB%xua(iPAR),
     &             sGLB%iopt_par(iPAR),sGLB%ivar_par(iPAR),
     &             sGLB%nsub_opt,sGLB%isub_opt)
      end if  !!if(sGLB%iModel.gt.0) then
!!SCE_END   
...
!      if(sGLB%iopt_par(iPAR).gt.0) then
!          do jua = 1, sGLB%nsub_opt
!              if(i.eq.sGLB%isub_opt(jua)) then
!                  if(sGLB%ivar_par(iPAR).eq.2) then  !relative value
!                    do j = 1, nly
!                        sol_k(j,ihru) = sol_k(j,ihru)*sGLB%xua(iPAR)  !relative value, multiplied by a factor
!                    end do
!                  else
!                    do j = 2, nly
!                        sol_k(j,ihru)=sGLB%xua(iPAR)*sol_k(j,ihru)/
!     &                          max(0.0001,sol_k(1,ihru))
!                    end do
!                    sol_k(1,ihru) = sGLB%xua(iPAR)  !absolute value
!                  end if
!              end if
!          end do
!      end if