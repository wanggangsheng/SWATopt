...
color1 <- c("gray", "cornflowerblue","darkgreen")
...
sp <- sp + labs(y="Area [%]") + scale_fill_manual(values=color1)
...
dev.off()
	...
	# if(Var[i]=="Water") {
	# 	ytitle <- bquote(paste(.(Var[i]),": ", .(Matrics[1]), " [mm]; ", .(Matrics[2])," [",m^3," ",s^-1,"]"))
	# }else {
	# 	ytitle <- bquote(paste(.(Var[i]),": ", .(Matrics[1]), " [kg ",ha^-1,"]; ", .(Matrics[2])," [mg ",L^-1,"]"))
	# }
	...
	# sp <- eval(bquote(ggplot(data2, aes(x=.(as.name(data2_colnames[4])), y=.(as.name(data2_colnames[3])), fill=.(as.name(data2_colnames[1])))) + ylim(0,ylim_max[i]) + geom_boxplot())) 
 #    sp <- sp + ggtitle(ytitle)

 #    sp <- sp + scale_fill_manual(values=color1)
