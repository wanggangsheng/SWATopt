...
ncase <- length(cases)
nscen <- ncase*(ncase - 1)/2
Scenario <- character(nscen)
k <- 0
for(i in 1:(ncase-1)){
	for(j in (i+1):ncase){
		k <- k + 1
		Scenario[k] = paste0(cases[j],".vs.",cases[i])
	}
...
for (i in 1:(ncase-1)){
	for(j in (i+1):ncase){
		k <- k + 1
		data_diff[[k]] <- data_new[[i]][,(ncol_skip+1):ncol0]
		data_diff[[k]] <- data_new[[j]][,(ncol_skip+1):ncol0] - data_new[[i]][,(ncol_skip+1):ncol0]

		data_diff_perc[[k]] <- data_diff[[k]]/data_new[[i]][,(ncol_skip+1):ncol0]*100  # %change

		write.table(round(data_diff[[k]],4),fn1[k],sep="\t",row.names=FALSE)
		write.table(round(data_diff_perc[[k]],4),fn2[k],sep="\t",row.names=FALSE)
	}
}

print(">>>t-test for data_diff")
nvar2 <- ncol(data_diff[[1]])
var2_name <- colnames(data_diff[[1]])
colClasses <- c("character", rep("numeric",nvar2))
# colClasses <- c(rep("character",nvar2+1))
colnames <- c("Scenario", var2_name)
ttest_diff <- read.table(text = "", colClasses = colClasses, col.names = colnames)
fn1 <- paste0(path_out,"/Load_diff_ttest_pvalue",".txt") 

for (k in 1:nscen){
	ttest_diff[k,1] <- Scenario[k]
	for(m in 1:nvar2){
		t_test1 <- t.test(data_diff[[k]][,m],mu=0)
		ttest_diff[k,m+1] <- round(t_test1[[3]],4)
	}
}
write.table(ttest_diff,fn1,sep="\t",row.names=FALSE)
# for (k in 1:nscen){
# 	ttest_diff[(k*2-1):(k*2),1] <- Scenario[k]
# 	for(m in 1:nvar2){
# 		t_test1 <- t.test(data_diff[[k]][,m],mu=0)
# 		ttest_diff[(k*2-1),m+1] <- round(t_test1[[3]],4)
# 		if(ttest_diff[(k*2-1),m+1]<0.001) {
# 			ttest_diff[k*2,m+1] <- "**"
# 		} else if (ttest_diff[(k*2-1),m+1]<0.05) {
# 			ttest_diff[k*2,m+1] <- "*"
# 		} else {
# 			ttest_diff[k*2,m+1] <- "NS"
# 		}
# 	}
# }
...
for (k in 1:3){
	...
	for (i in 1:nvar){
		irow_beg <- (i-1)*(nscen-1)*nsub + 1
		irow_end <- irow_beg + (nscen-1)*nsub - 1
