...
for (i in 1:(ncase-1)){
	for(j in (i+1):ncase){
		k <- k + 1
		data_diff[[k]] <- data_conc[[i]]
		data_diff[[k]] <- data_conc[[j]] - data_conc[[i]]

		data_diff_perc[[k]] <- data_diff[[k]]/data_conc[[i]]*100  # %change

		write.table(round(data_diff[[k]],4),fn1[k],sep="\t",row.names=FALSE)
		write.table(round(data_diff_perc[[k]],2),fn2[k],sep="\t",row.names=FALSE)
	}
}
...
fn1 <- paste0(path_out,"/Conc_diff_ttest_pvalue",".txt") 
