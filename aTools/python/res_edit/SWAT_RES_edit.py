...
len_max = 7
WURESN_mon = [1.00,1.00,1.00,1.00,1.00,1.00,1.00,1.00,1.00,1.00,1.00,1.00]
# WURESN_mon = [0.95,0.97,0.93,1.10,1.01,1.14,1.07,1.02,1.03,1.05,0.86,0.88]
print
print (">>>>>>>>>>>>>>>>>>>>>>>>>>")
...
##    print(lines[iLine - 1])
    ...
    iLine = 34 ##RWURESN[1:6]
    len1 = len(lines[iLine - 1])
    lines[iLine - 1] = '%10.1f'%(WURESN*WURESN_mon[0])
    for i in range(1,6):
        lines[iLine - 1] = lines[iLine - 1] + '%10.1f'%(WURESN*WURESN_mon[i])
    lines[iLine - 1] = lines[iLine - 1]+'\n'

    iLine = 36 ##RWURESN[7:12]
    len1 = len(lines[iLine - 1])
    lines[iLine - 1] = '%10.1f'%(WURESN*WURESN_mon[6])
    for i in range(7,12):
        lines[iLine - 1] = lines[iLine - 1] + '%10.1f'%(WURESN*WURESN_mon[i])
    lines[iLine - 1] = lines[iLine - 1]+'\n'
    ...
    iLine = 37 ##WURTNF
    len1 = len(lines[iLine - 1])
    lines[iLine - 1] = '%16.2f'%WURTNF + lines[iLine - 1][16:len1]
##    print(lines[iLine - 1])
