...
file_master = dir_master + "master.ini"
file = open(file_master,'r')
...
line = lines[1].strip() ##lines[0]: head; lines[1]:data
data1 = line.split()
nCOL1 = len(data1)
##print("nCOL in file_mater_PAR = ",nCOL1)
PAR_info = np.empty([nPAR,nCOL1],dtype=np.object) ##nPAR<=nPAR_all

for j in range(nPAR):
    for i in range(1,nPAR_all+1):
        line = lines[i].strip()
        data1 = line.split() ##['1' 'CN2' 'mgt' '11' '16.2' 'HRU' '1' '0']
        if(data1[1]==PAR_name[j]):
            PAR_info[j] = data1
    if(PAR_info[j,0]==None):
        print("ERROR:",PAR_name[j]," is NOT in ",file_master_PAR)
        sys.exit()
        
##print("PAR_info = ", PAR_info) ##PAR_info only includes those PAR in HUC8_PAR
...
for j in range(nHUC8):
    for i in range(nHUC8_all):
        ...
        if(data1[0]==HUC8_PAR[j,0]):
            HUC8_SUB[j] = data1
    if(HUC8_SUB[j,0]==None):
        print("ERROR:",HUC8_PAR[j,0]," is NOT in ",file_master_HUC8_SUB)
        sys.exit()
...
##    k = -1
    for j1 in range(sArg.npar):
        ...
        for j2 in range(nPAR):              
            if(PAR_name[j2]==PAR_info[iROW,1]):
                iCOL_HUC8_PAR[j1] = j2 + 1  #1st column in HUC8_PAR is HUC8
    ...
    sArg.par_update = np.empty(sArg.npar,dtype=np.int) ###sArg
    ##PAR_info[0,:] = ['1' 'CN2' 'mgt' '11' '16.2' 'HRU' '1']
    for j1 in range(sArg.npar):
        iROW = iROW_PAR_info[j1]
        ...
        sArg.par_update[j1] = PAR_info[iROW,7]  ###sArg, 1-update, 0-no update
...
##                print (sArg.__dict__)
                if os.path.exists(sArg.file_in):  ###file may not exit for *.lwq (reservoir)
                    fwrite_swat_par(sArg)
            elif (SWAT_unit[i]=='BSN'):
                fname1 = 'basins.bsn'
                sArg.file_in = dir_in + fname1  ###sArg
                sArg.file_out = dir_out + fname1  ###sArg
                fwrite_swat_par(sArg)                
