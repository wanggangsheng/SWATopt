...
def fwrite_swat_par(swrite_swat_par):
    ...
    par_update = swrite_swat_par.par_update
##    print('file_in = ',file_in)
    ...
    for i in range(npar):
        if par_update[i] == 1:  ##update the parameter, otherwirse no update
            j = irow[i] - 1
            len1 = len(lines[j])
    ...
    ##                    print(k,par_all2[k],par_all2[0],par_all3[k],par_all_new[k])
                ...
                lines[j] = line_new + "\n"           
            elif (option[i]==4):  ##multiple layers for 1 PAR, but the same value, e.g., *.chm 
                for k in range(len1):
                    if(lines[j][k]==":"):
                        par_pos0 = k+1
                ...
                len2 = len(par_all2)  ## of layers
    ##            par_all3 = np.full(len2,0,dtype = np.float)
                par_all_new = np.full(len2,par_val[i],dtype = np.float)
    ##            par_all_new[0] = par_val[i]
                line_new = lines[j][:par_pos0]
                for k in range(len2):
                    line_new = line_new+(par_format[i])%(par_all_new[k])
                lines[j] = line_new + "\n"
            elif (option[i]==3):  ##1 layer for 1 PAR, 'SUB' but need to update 2 lines:current line +next line; e.g., PAR: NSETLR,PSETLR in *.lwq  
                line_new = lines[j][:(par_pos0[i]-1)]
                pos1 = (par_pos0[i] - 1 + par_len[i])
                lines[j] = line_new+(par_format[i])%(par_val[i]) + lines[j][pos1:len1]

                line_new = lines[j+1][:(par_pos0[i]-1)]
                pos1 = (par_pos0[i] - 1 + par_len[i])
                lines[j+1] = line_new+(par_format[i])%(par_val[i]) + lines[j+1][pos1:len1]    
            else:  ##option[i]=1, ##1 layer for 1 PAR, e.g., *.gw, sub.lag
                line_new = lines[j][:(par_pos0[i]-1)]
                pos1 = (par_pos0[i] - 1 + par_len[i])
                lines[j] = line_new+(par_format[i])%(par_val[i]) + lines[j][pos1:len1]
